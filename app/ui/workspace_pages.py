...
class MatchingPage(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self.item_table.verticalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Fixed)
        self.item_table.verticalHeader().setDefaultSectionSize(36)
        self.item_table.verticalHeader().setMinimumWidth(48)
        self.item_table.verticalHeader().setFixedWidth(48)
...
class CoverPage(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        action_row.addWidget(QLabel("下载并发"))
        self.download_concurrency_combo = QComboBox()
        for value in (1, 2, 3, 4):
            self.download_concurrency_combo.addItem(f"{value} 路", value)
        self.download_concurrency_combo.setCurrentIndex(3)
        self.download_concurrency_combo.setToolTip("独立下载封面时的并发路数，建议 2-4 路")
        action_row.addWidget(self.download_concurrency_combo)
        action_row.addWidget(QLabel("检测并发"))
        self.review_concurrency_combo = QComboBox()
        for value in (1, 2, 3):
            self.review_concurrency_combo.addItem(f"{value} 路", value)
        self.review_concurrency_combo.setCurrentIndex(2)
        self.review_concurrency_combo.setToolTip("模型检测封面时的并发路数，建议不超过 3 路")
        action_row.addWidget(self.review_concurrency_combo)
        ...
        select_remaining_button = QPushButton("选择至末尾")
        select_remaining_button.setProperty("secondary", True)
        select_remaining_button.setToolTip("从当前页最靠前的已勾选条目开始，一直选择到列表末尾")
        select_remaining_button.clicked.connect(self._select_to_end)
        selection_bar.addWidget(select_remaining_button)
    ...
    def _render_queue_table(self) -> None:
        ...
        for row, source_index in enumerate(self._visible_indices):
            ...
            title_item = QTableWidgetItem(str(video.get("title") or ""))
            title_item.setToolTip(title_item.text())
            self.item_table.setItem(row, 1, title_item)
            video_id_item = QTableWidgetItem(str(video.get("video_id") or ""))
            video_id_item.setToolTip(video_id_item.text())
            self.item_table.setItem(row, 2, video_id_item)
            channel_item = QTableWidgetItem(str(video.get("channel") or ""))
            channel_item.setToolTip(channel_item.text())
            self.item_table.setItem(row, 3, channel_item)
        ...
        self.item_table.setVerticalHeaderLabels([str(index + 1) for index in self._visible_indices])
    ...
    def _select_to_end(self) -> None:
        selected = self._selected_rows()
        if not selected:
            self.set_status("请先勾选起始条目，再点击“选择至末尾”。")
            return
        start = min(selected)
        self._set_rows_checked(set(range(start, self.item_table.rowCount())))
        self.set_status(f"已从第 {start + 1} 条选择至当前页末尾，共 {self.item_table.rowCount() - start} 条。")
